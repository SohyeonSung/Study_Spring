...
public class BoardController {
	...
	public String insertBoard(BoardVO vo) throws IllegalStateException, IOException {
		...
		if (uploadFile == null) {
			System.out.println(":: uploadFile 요청 파라미터가 없음");
		} else if (uploadFile.isEmpty()) {
			System.out.println(":: 전달받은 파일 데이터가 없음");
		} else {
			System.out.println("uploadFile.isEmpty() : " + uploadFile.isEmpty());
			System.out.println(":: 파일업로드 작업진행~~~~");
			String fileName = uploadFile.getOriginalFilename(); // 원본파일명
			System.out.println("- 원본파일명 : " + fileName);
			String savedFileName = UUID.randomUUID().toString(); // 물리적 저장할파일명
